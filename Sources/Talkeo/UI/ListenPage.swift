...
struct ListenPage: View {
    ...
    private var playbackArea: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            ListenPageTransport(text: model.sourceText, detectedLang: model.detectedLang, speechRate: $model.speechRate)
...
/// Playback transport for the app page: a seekable timeline, play/pause/stop
/// and speed, driven by the real Talkeo voice (`TTSAudioPlayer`). Same
/// behavior as the popover's own transport — kept as its own copy rather than
/// shared, since both are due a real visual pass later ("no está nada bueno").
private struct ListenPageTransport: View {
    let text: String
    ...
    private var timeline: some View {
        GeometryReader { geo in
            ...
            ZStack(alignment: .leading) {
                ...
                Capsule().fill(Palette.primary).frame(width: x, height: 5)
                Circle()
                    .fill(.white)
